 ...
 * operaciones que se realizan sobre los Accesos
 *
 * @author Julio
 * @version 1.0
 * @since 2016-05-07
...
public class AccesoResource {
    ...
    /**
     * Metodo listAccesos, retorna un PaginatedListWrapper con accesos del
     * perfil
     *
    ...
    /**
     * Metodo consultarAcceso, devuelve un objeto de tipo Acceso que trae desde
     * el facade con un id
     *
    ...
    @Path("{acceAcce}")
    @Produces(MediaType.APPLICATION_JSON)
    public Acceso consultarAcceso(@PathParam("acceAcce") Long id) throws CustomException, CustomRunTimeException {
        try {
            return facadeAcceso.find(id);
        } catch (Exception ex) {
            throw new CustomException(
                    Response.Status.BAD_REQUEST.getStatusCode(), 603,
                    "Error consultando Acceso... ");
        }
    }

    /**
     * Metodo eliminarAcceso, elimina el objeto de tipo Acceso con el id en
     * referencia
     *
    ...
    @DELETE
    @Path("{acceAcce}")
    public void eliminarAcceso(@PathParam("acceAcce") Long id) throws CustomException, CustomRunTimeException {
        try {
    ...
    /**
     * Metodo guardarAcceso, crea y modifica el objeto de tipo Acceso del objeto
     * en referencia
     *
     * @param acceso es el objeto a crear o modificar que llega, si este objeto
     * en su parte id es nulo se crea, sino se modifica
     */
    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    public void guardarAcceso(Acceso acceso) throws CustomException, CustomRunTimeException {
        try {
