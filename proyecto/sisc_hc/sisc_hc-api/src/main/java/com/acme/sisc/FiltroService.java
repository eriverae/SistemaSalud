...
public class FiltroService {
    ...
    @EJB
    IFiltroFacadeRemote facadefiltro;

    @GET
    @Produces({"application/json"})
    public Response GetMedicosTodasCitas(@QueryParam("idcita") String idcita)
            throws CustomException, CustomRunTimeException {
        try {

            HashMap m = new HashMap();
            m.put("data", facadefiltro.findByCita(Long.parseLong(idcita)));
            return Response
                    .status(200)
                    .entity(m)
                    .build();
            ...
            throw new CustomException(Response.Status.BAD_REQUEST.getStatusCode(), 503, "Error accediendo a los datos de todos los medicos de citas... ");
        }
    }
}
